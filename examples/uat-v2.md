# Legate v2.0 UAT — Full Acceptance Test
...
2. `legate-mcp` is built: `npm run build` in the supervisor repo
3. MCP server is registered in `.mcp.json` (or use `legate init`)
...
### T1.1 — `legate init` writes `.mcp.json`
...
**Pass:** `.mcp.json` created. Contains a `legate` server entry with `build/index.js` or the installed bin path.  
**Fail:** File not created, or error output.

---

### T1.2 — `legate add-server` registers a named server

```bash
legate add-server thor localhost 4096 vllm "Qwen/Qwen3-Coder-30B-A3B-Instruct"
```
...
### T1.3 — `legate add-server` with `--max-sessions` capacity flag

```bash
legate add-server lab localhost 4097 mlx "mlx-community/Qwen3-Coder-Next-4bit" --max-sessions 2
```
...
### T1.4 — `legate list-servers` displays all registered servers
...
### T1.5 — `legate remove-server` deregisters a server

```bash
legate remove-server lab
```
...
```bash
legate add-server bad localhost 99999 vllm model
```
...
```bash
legate add-server bad localhost 4096 vllm model --max-sessions 1.5
```
...
### T2.1 — `legate_create_session` with named server routes to correct URL

```
legate_create_session({ title: "UAT session thor", server: "thor", directory: "/tmp/legate-uat" })
```
...
### T2.2 — `legate_create_session` without server uses first-in-registry (thor)

```
legate_create_session({ title: "UAT default routing", directory: "/tmp/legate-uat" })
```
...
### T2.3 — `legate_create_session` with unknown server returns clear error

```
legate_create_session({ title: "bad server", server: "does-not-exist" })
```

**Pass:** Error: `Server 'does-not-exist' not found in registry. Run 'legate list-servers'…`  
**Fail:** Generic connection error or silent failure.

---

### T2.4 — `legate_session_list` returns active sessions

`legate_session_list` without `directory` returns ALL sessions on the OpenCode server regardless of project. Pass `directory` to scope to a specific project.

```
legate_session_list({ directory: "/tmp/legate-uat" })
```
...
### T3.2 — `legate_get_diff` shows file changes after run

```
legate_get_diff({ sessionId: SESSION_ID_THOR })
```
...
### T3.3 — `legate_run` correction loop (follow-up prompt)
...
**Pass:** Responds without error. `hello.ts` now contains `UAT hello v2` (verify with `cat /tmp/legate-uat/hello.ts`).

---

### T3.4 — `legate_session_messages` returns message history

```
legate_session_messages({ sessionId: SESSION_ID_THOR })
```
...
### T3.5 — `legate_session_message` returns a single message by ID

Take any `id` from T3.4, e.g., `MSG_ID_1`.

```
legate_session_message({ sessionId: SESSION_ID_THOR, messageID: MSG_ID_1 })
```
...
### T4.1 — `legate_revert` undoes the last message

Take the ID of the last user message from `legate_session_messages`.

```
legate_revert({ sessionId: SESSION_ID_THOR, messageID: LAST_MSG_ID })
```

**Pass:** Returns success. Session object has `revert.messageID` set (the virtual tip is moved back). Note: OpenCode 1.14.33 does NOT hide reverted messages from the messages list endpoint — message count will be unchanged. Verify by calling `legate_session_get` and confirming the `revert` field is non-null.

---

### T4.2 — `legate_session_unrevert` restores a reverted message

```
legate_session_unrevert({ sessionId: SESSION_ID_THOR })
```
...
### T4.3 — `legate_fork` creates a new session at a checkpoint

Take a message ID from early in the session (first assistant reply), `FORK_MSG_ID`.

```
legate_fork({ sessionId: SESSION_ID_THOR, messageID: FORK_MSG_ID })
```

**Pass:** Returns new session object with a different `id`. New session has messages only up to `FORK_MSG_ID`. Note: `directory` in the forked session reflects the OpenCode server's working directory, not necessarily `/tmp/legate-uat`.  
**Save:** `SESSION_ID_FORK = <returned id>`

---

### T4.4 — `legate_session_children` shows fork lineage

```
legate_session_children({ sessionId: SESSION_ID_THOR })
```
...
### T4.5 — `legate_abort` cancels an in-flight run (manual test)

Start a long run, then abort mid-flight:
```
legate_run({ sessionId: SESSION_ID_FORK, prompt: "Write a 500-line TypeScript module with detailed comments for every function." })
```
Immediately call:
```
legate_abort({ sessionId: SESSION_ID_FORK })
```

**Pass:** Abort returns success. The `legate_run` call either returns with partial output or returns error mentioning abort. Server is not left in a hung state.  
...
### T5.1 — `legate_delegate` creates a session and runs a prompt atomically
...
**Pass:** Returns `{ sessionId, response }`. Session is created, prompt executed. `delegate-test.ts` exists in `/tmp/legate-uat`.  
**Save:** `SESSION_ID_DELEGATE = <returned sessionId>`

---

### T5.2 — `legate_delegate` with existing `sessionId` reuses the session
...
**Partial:** If the call times out after LEGATE_TIMEOUT_MS and returns `isError: true` with "run aborted" — this is a model-speed issue, not a code defect. The session is kept alive and the in-flight run is aborted to prevent cascading busy state. Increase `LEGATE_TIMEOUT_MS` in `.mcp.json` if model is consistently slow.

---

### T5.3 — `legate_dispatch` sends a fire-and-forget prompt
...
### T5.4 — `legate_await` polls until dispatch completes

```
legate_await({ sessionId: SESSION_ID_DISPATCH })
```
...
### T5.5 — `legate_inspect` returns full session state

```
legate_inspect({ sessionId: SESSION_ID_DISPATCH })
```

**Pass:** Returns a compact snapshot `{ status, todos, changedFiles }`. No `isError: true`. (`legate_inspect` is not a full session object — use `legate_session_get` for full session details.)

---

### T5.6 — `legate_session_status` returns running/idle status

`legate_session_status` takes no `sessionId` — it returns a map of ALL sessions. `directory` is the only optional parameter.

```
legate_session_status({ directory: "/tmp/legate-uat" })
```
...
```bash
legate add-server lab localhost 4096 vllm "Qwen/Qwen3-Coder-30B-A3B-Instruct" --max-sessions 1
```
...
```
legate_create_session({ title: "cap test 1", server: "lab", directory: "/tmp/legate-uat" })
```
...
```
legate_create_session({ title: "cap test 2", server: "lab" })
```
...
```
legate_session_delete({ sessionId: SESSION_ID_CAP1 })
```

Then retry:
```
legate_create_session({ title: "cap test 2 retry", server: "lab", directory: "/tmp/legate-uat" })
```
...
### T6.5 — Capacity enforced on `legate_delegate` entry point
...
### T6.6 — Capacity enforced on `legate_dispatch` entry point
...
### T7.1 — `legate_session_rename` changes the session title

```
legate_session_rename({ sessionId: SESSION_ID_THOR, title: "UAT Thor (renamed)" })
```

**Pass:** `legate_session_get({ sessionId: SESSION_ID_THOR })` returns `title: "UAT Thor (renamed)"`.

---

### T7.2 — `legate_session_get` returns full session details

```
legate_session_get({ sessionId: SESSION_ID_THOR })
```

**Pass:** Returns session object with `id` matching SESSION_ID_THOR. Note: `messages` is not a top-level field on the session object — use `legate_session_messages` to retrieve message history separately.

---

### T7.3 — `legate_session_summarize` accepts the request
...
**Pass:** Returns `true` (async acceptance). No `isError: true`. The summary is generated asynchronously by the model — poll `legate_session_messages` to retrieve it once the session is idle.

---

### T7.4 — `legate_session_todo` returns a TODO list

```
legate_session_todo({ sessionId: SESSION_ID_THOR })
```
...
### T7.5 — `legate_session_command` runs a slash command

First check what commands exist:
```
legate_list_commands({ directory: "/tmp/legate-uat" })
```
...
**Fail:** Error, or agent name mismatch error (check `legate_list_agents` for valid agent names).

---

### T7.7 — `legate_session_init` creates AGENTS.md
...
### T7.8 — `legate_session_init` guard fires when AGENTS.md exists
...
### T7.9 — `legate_session_init` with `force: true` bypasses guard
...
### T7.10 — `legate_session_init` rejects UUID as messageID
...
### T7.11 — `legate_session_share` makes a session shareable

```
legate_session_share({ sessionId: SESSION_ID_THOR })
```
...
### T7.12 — `legate_session_unshare` removes sharing

```
legate_session_unshare({ sessionId: SESSION_ID_THOR })
```

**Pass:** Returns success. Subsequent `legate_session_get` shows `share` field as null or removed.

---

## Part 8 — API Introspection Tools

### T8.1 — `legate_list_providers` returns configured providers

```
legate_list_providers({ directory: "/tmp/legate-uat" })
```
...
### T8.2 — `legate_list_agents` returns configured agents

```
legate_list_agents({ directory: "/tmp/legate-uat" })
```
...
### T8.3 — `legate_find_symbol` finds a TypeScript symbol
...
### T8.4 — `legate_find_file` finds files by name pattern
...
### T8.5 — `legate_get_file_content` returns file content
...
### T8.6 — `legate_vcs_info` returns git branch

```
legate_vcs_info({ directory: "/tmp/legate-uat" })
```
...
### T8.7 — `legate_file_status` returns modified files
...
### T8.8 — `legate_list_mcp_servers` returns MCP server status map
...
**Note:** MCP server visibility depends on the OpenCode host's project-level config. An empty map `{}` is acceptable if no MCP servers are configured for this project on the OpenCode host — this is not a Legate defect.

---

### T8.9 — `legate_get_config` returns OpenCode config

```
legate_get_config({ directory: "/tmp/legate-uat" })
```
...
### T8.10 — `legate_list_commands` returns slash commands
...
### T8.11 — `legate_list_tools` without provider/model returns all tool IDs
...
### T8.12 — `legate_list_tools` with provider+model returns tool details
...
### T8.13 — `legate_list_tools` with only provider (no model) returns error

```
legate_list_tools({ provider: "vllm" })
```
...
legate_inject_mcp_server({
  ...
  configType: "local",
  commandArgs: ["node", "/tmp/legate-uat/nonexistent.js"],
  enabled: false
})
```
...
**Fail:** Error thrown by Legate layer (not by OpenCode responding with a known error status).

---

### T8.15 — `legate_inject_mcp_server` without commandArgs rejects local config
...
### T8.16 — `legate_prompt_async` sends non-blocking prompt
...
```
legate_run({ sessionId: "nonexistent_session_id_xyz", prompt: "test" })
```
...
### T9.2 — `legate_session_delete` removes session from registry

```
legate_session_delete({ sessionId: SESSION_ID_DEFAULT })
```

**Pass:** Returns success. `legate_session_list` no longer includes SESSION_ID_DEFAULT.
...
### T10.1 — `legate_create_session` autostarts OpenCode when server is down
...
Then immediately:
```
legate_create_session({ title: "autostart test", server: "thor", directory: "/tmp/legate-uat" })
```

**Pass:** Session created successfully (Legate spawned `opencode serve`, health-polled, then connected). May take up to 30s.  
...
7. `legate_find_symbol` (T8.3) is non-functional in OpenCode ≤ 1.14.33 — the `/find/symbol` endpoint returns `[]` because `workspace/symbol` LSP requests are not yet implemented. Mark T8.3 as SKIP on this version.
