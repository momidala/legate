...
test('readSessionMap prunes entries older than LEGATE_SESSION_TTL_MS', async () => {
  const dir = freshTmp();
  try {
    const regPath = join(dir, 'sessions.json');
...
test('readSessionMap keeps legacy entries without createdAt (never expires them)', async () => {
  const dir = freshTmp();
  try {
    const regPath = join(dir, 'sessions.json');
    writeSessionMap(
...
test('readSessionMap is side-effect free: does NOT rewrite the file when it contains expired entries', async () => {
  const dir = freshTmp();
  try {
    const regPath = join(dir, 'sessions.json');
    const old = Date.now() - 1000; // 1 second ago
  ...
  try {
    ...
    try {
      ...
      ({ warnings } = await captureWarnings(() =>
        assert.doesNotReject(removeSession('ses_keep', regPath, { retries: 0 })),
      ));
    } finally {
      await release();
    }
...
test('readSessionMap emits one-time deprecation warning for PREFECT_SESSION_TTL_MS when LEGATE_SESSION_TTL_MS is not set', async () => {
  _resetWarnFlags();
  const dir = mkdtempSync(join(tmpdir(), 'prefect-sessions-'));
  try {
    await withEnv({ LEGATE_SESSION_TTL_MS: undefined, PREFECT_SESSION_TTL_MS: '3600000' }, async () => {
      const regPath = join(dir, 'sessions.json');
...
test('readSessionMap emits PREFECT_SESSION_TTL_MS warning exactly once when called twice (one-time-per-process guard)', async () => {
  _resetWarnFlags();
  const dir = mkdtempSync(join(tmpdir(), 'prefect-sessions-'));
  try {
    await withEnv({ LEGATE_SESSION_TTL_MS: undefined, PREFECT_SESSION_TTL_MS: '3600000' }, async () => {
...
test('readSessionMap emits NO PREFECT_SESSION_TTL_MS warning when LEGATE_SESSION_TTL_MS is also set (LEGATE_* takes precedence)', async () => {
  _resetWarnFlags();
  const dir = mkdtempSync(join(tmpdir(), 'prefect-sessions-'));
  try {
    await withEnv({ LEGATE_SESSION_TTL_MS: '2000', PREFECT_SESSION_TTL_MS: '1000' }, async () => {
      const regPath = join(dir, 'sessions.json');
      const { warnings } = await captureWarnings(() => readSessionMap(regPath));
      const ttlWarnings = warnings.filter((w) => w.includes('PREFECT_SESSION_TTL_MS'));
      assert.equal(
